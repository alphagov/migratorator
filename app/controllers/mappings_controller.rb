    ...
    respond_to do |format|
      ...
      format.csv do 
        csv = CSV.generate do |csv|
          csv << ["Title", "Old Url", "New Url", "Status", "Notes", "Group", "Name", "Whole Tag", "Suggested Links", "Archive Link"]
          apply_scopes(Mapping).all.each do |mapping|
            ...
            csv << [mapping.title, mapping.old_url, mapping.new_url, mapping.status, mapping.notes, tag_group.join(' '), tag_name.join(' '), tag_whole_tag.join(' '), mapping.alternative_urls, mapping.archive_url]
          end
        end
